  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <!-- Refresh button: Emits 'refresh' event to parent. Disabled when loading is true. -->
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" title="Refresh Graph">
          <!-- Spinning animation class applied conditionally when loading -->
          <span class="icon-refresh" :class="{ 'spinning': loading }">↻</span>
          <span class="btn-text">Refresh</span>
        </button>
        <!-- Maximize button: Emits 'toggle-maximize' to toggle fullscreen mode -->
    ...
    <div class="graph-container" ref="graphContainer">
      <!-- Graph visualization -->
      <!-- Only render the SVG area if graphData exists -->
      <div v-if="graphData" class="graph-view">
        <svg ref="graphSvg" class="graph-svg"></svg>
        
        <!-- Building/Simulating hint -->
        <!-- Overlay hint shown during Phase 1 (building) or when simulation is active -->
        ...
        <!-- Dismissible hint that appears when a simulation run finishes -->
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still being processed. It is recommended to manually refresh the graph later.</span>
          <!-- Close button resets the local state variable -->
        ...
        <!-- Floating panel on the right side showing details of the clicked element -->
        <div v-if="selectedItem" class="detail-panel">
          <div class="detail-panel-header">
            <span class="detail-title">{{ selectedItem.type === 'node' ? 'Node Details' : 'Relationship' }}</span>
            <!-- Dynamic badge showing the entity type with corresponding color -->
          ...
          <div v-if="selectedItem.type === 'node'" class="detail-content">
            ...
            <div class="detail-row">
              ...
              <!-- specific class for monospace display of IDs -->
            ...
            <!-- Properties -->
            <!-- Iterating over arbitrary attributes key-value pairs -->
          ...
          <div v-else class="detail-content">
            ...
            <!-- Special handling for self-loops which are grouped into a list -->
            <template v-if="selectedItem.data.isSelfLoopGroup">
              ...
              <div class="self-loop-list">
                <!-- Collapsible items for each self-loop edge -->
                <div 
    ...
    <!-- Legend keys generated dynamically based on the unique entity types in the graph -->
    ...
    <!-- Control to show/hide edge labels to reduce visual clutter -->
...
<script setup>
// Importing necessary functions from Vue and D3 library
import { ref, onMounted, onUnmounted, watch, nextTick, computed } from 'vue'
import * as d3 from 'd3'

// Defining component props passed down from the parent
const props = defineProps({
  graphData: Object, // The core graph object containing nodes and edges
  loading: Boolean,   // Loading state indicator
  currentPhase: Number, // Current processing phase (e.g., 1 for building)
  isSimulating: Boolean // Whether the simulation engine is currently running
})

// Defining custom events this component emits to the parent
const emit = defineEmits(['refresh', 'toggle-maximize'])

// Template refs to access DOM elements directly for D3 rendering
const graphContainer = ref(null)
const graphSvg = ref(null)
const selectedItem = ref(null) // Stores the currently selected node or edge for the detail panel
const showEdgeLabels = ref(true) // Show edge labels by default
const expandedSelfLoops = ref(new Set()) // Tracks which self-loop items are expanded in the UI using a Set for O(1) lookups
const showSimulationFinishedHint = ref(false) // Hint shown after simulation ends
const wasSimulating = ref(false) // Track whether previously simulating to detect state transitions
...
// Listen for isSimulating changes, detect when simulation ends
// This logic detects the "falling edge" of the simulation state (true -> false)
...
// This creates a unique list of entity types and assigns them colors dynamically
const entityTypes = computed(() => {
  ...
  props.graphData.nodes.forEach(node => {
    // Extract the specific type label, fallback to 'Entity'
...
// Variables to hold the D3 simulation instance and label references globally within the component
...
const renderGraph = () => {
  ...
  // Stop previous simulation to prevent overlapping physics calculations
  ...
  // Initialize the SVG canvas using D3 selections
  const svg = d3.select(graphSvg.value)
    .attr('width', width)
    .attr('height', height)
    .attr('viewBox', `0 0 ${width} ${height}`)
    
  // Clear any existing graph elements before re-rendering
  ...
  // Prep data
  // Create a quick lookup map for nodes by UUID
  const nodeMap = {}
  nodesData.forEach(n => nodeMap[n.uuid] = n)
  
  // Transform raw data into D3-compatible format
  const nodes = nodesData.map(n => ({
    id: n.uuid,
    name: n.name || 'Unnamed',
    type: n.labels?.find(l => l !== 'Entity') || 'Entity',
    rawData: n // Preserve raw data for detail panel
  }))
  
  const nodeIds = new Set(nodes.map(n => n.id))
  
  // Process edge data, calculate edge count and index between the same pair of nodes
  // This logic is needed to prevent overlapping edges (multi-edges) between the same nodes
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      ...
      // Create a unique key for the pair (undirected) to count parallel edges
      const pairKey = [e.source_node_uuid, e.target_node_uuid].sort().join('_')
      edgePairCount[pairKey] = (edgePairCount[pairKey] || 0) + 1
    }
  })
  
  // Record which edge index is currently being processed for each node pair
  const edgePairIndex = {}
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // Base distance 150, increase by 50 per additional edge
    ...
    .force('collide', d3.forceCollide(50)) // Collision detection to prevent overlap
    // Add gravitational pull toward center to cluster isolated node groups into the center area
    .force('x', d3.forceX(width / 2).strength(0.04))
    .force('y', d3.forceY(height / 2).strength(0.04))
  
  currentSimulation = simulation

  // Main container group for zoom/pan transformations
  ...
  // Helper function to generate SVG path strings for edges (straight, curved, or self-loop)
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc starting from the node and returning to it
      const loopRadius = 30
    ...
    // Perpendicular offset to the line direction, calculated proportionally to distance, ensuring the curve is clearly visible
    // The more edges there are, the larger the offset ratio relative to distance
  ...
  // Calculate curve midpoint (used for label positioning)
  ...
  // Render the edges (links) as SVG paths
  ...
  // Render a rect behind the text for better readability
  ...
  // Save references for external visibility control (used by the watcher)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position, do not restart simulation (distinguishes click from drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect if a real drag has started (moved beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // First time detecting a real drag, restart simulation
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only let simulation gradually stop if a real drag occurred
        if (d._isDragging) {
          simulation.alphaTarget(0)
        }
        // Clear fixed position to allow simulation to take over again
        d.fx = null
        d.fy = null
        d._isDragging = false
      })
    ...
    .on('mouseenter', (event, d) => {
      // Hover effect, unless it's the selected node
  ...
  const nodeLabels = nodeGroup.selectAll('text')
    .data(nodes)
    .enter().append('text')
    .text(d => d.name.length > 8 ? d.name.substring(0, 8) + '…' : d.name) // Truncate long names
    .attr('font-size', '11px')
    .attr('fill', '#333')
    .attr('font-weight', '500')
    .attr('dx', 14)
    .attr('dy', 4)
    .style('pointer-events', 'none') // Allow clicks to pass through to the node circle
    .style('font-family', 'system-ui, sans-serif')

  // The 'tick' function updates the visual positions of all elements after every simulation step
  simulation.on('tick', () => {
    // Update curve path
  ...
  // Click on blank area to close the details panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// Watch for changes in graphData to re-render the graph
watch(() => props.graphData, () => {
  nextTick(renderGraph)
}, { deep: true })

// Listen for edge label display toggle
watch(showEdgeLabels, (newVal) => {
  // Directly manipulate DOM via D3 references for performance instead of re-rendering
  if (linkLabelsRef) {
...
// Cleanup event listeners and simulation on unmount to prevent memory leaks
onUnmounted(() => {
...
<style scoped>
/* Main container styles with a dot grid background */
.graph-panel {
  position: relative;
  width: 100%;
  height: 100%;
...
.panel-header {
  ...
  pointer-events: none; /* Allows clicking through the header empty space */
}
