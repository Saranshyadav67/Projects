<template>
  <!-- Root container for the entire process page -->
  <div class="process-page">
    <!-- Top Navigation Bar -->
    <nav class="navbar">
      <!-- Brand Logo: Clicking triggers navigation back to home -->
      <div class="nav-brand" @click="goHome">MIROFISH</div>
      
      <!-- Center Step Indicator: Shows current workflow step -->
      <div class="nav-center">
        <!-- Badge displaying the step number -->
        <div class="step-badge">STEP 01</div>
        <!-- Text displaying the step name -->
        <div class="step-name">Graph Construction</div>
      </div>

      <!-- Right side status indicator -->
      <div class="nav-status">
        <!-- Status dot: CSS class changes color based on state -->
        <span class="status-dot" :class="statusClass"></span>
        <!-- Status text: Display friendly status string -->
        <span class="status-text">{{ statusText }}</span>
      </div>
    </nav>

    <!-- Main Content Area: Split into Left (Graph) and Right (Process) panels -->
    <div class="main-content">
      <!-- Left Panel: Real-time Graph Display -->
      <!-- Binds 'full-screen' class if isFullScreen is true -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            <!-- Template condition: Only show stats if graphData exists -->
            <template v-if="graphData">
              <!-- Display node count with fallback logic (node_count property or array length or 0) -->
            ...
            <div class="action-buttons">
                <!-- Refresh button: Disabled when loading, shows spinning animation when loading -->
                ...
                <!-- Fullscreen toggle button: Icon changes based on state -->
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-if="graphData" class="graph-view">
            <!-- The actual SVG element manipulated by D3.js -->
            <svg ref="graphSvg" class="graph-svg"></svg>
            <!-- Building Hint: Shows real-time update status during phase 1 -->
            <div v-if="currentPhase === 1" class="graph-building-hint">
              <span class="building-dot"></span>
              Updating in real-time...
            </div>
            
            <!-- Node/Edge Detail Panel: Shows on click, positioned absolute top-right -->
            <div v-if="selectedItem" class="detail-panel">
              <div class="detail-panel-header">
                <!-- Dynamic title based on selection type -->
                <span class="detail-title">{{ selectedItem.type === 'node' ? 'Node Details' : 'Relationship' }}</span>
                <!-- Badge showing entity type (e.g., Person, Org) with dynamic background color -->
              ...
              <div v-if="selectedItem.type === 'node'" class="detail-content">
                ...
                <div class="detail-row">
                  ...
                  <!-- UUID class for monospace styling -->
                ...
                <!-- Properties / Attributes: Only render if attributes exist and are not empty -->
              ...
              <div v-else class="detail-content">
                <!-- Relationship Visual Representation: Source -> Type -> Target -->
          ...
          <!-- Loading State: Shown when graphLoading is true and no data exists yet -->
          ...
          <!-- Waiting for Construction: Shown if phase < 1 (Ontology not done) -->
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            <div class="waiting-icon">
              <!-- Inline SVG for network icon -->
            ...
            <p class="waiting-hint">Construction will start automatically after generation</p>
          </div>
          
          <!-- Building but no data yet: Intermediate state -->
        ...
        <!-- Graph Legend: Shows entity type color mapping -->
      ...
      <!-- Right Panel: Construction Process Details -->
      <!-- 'hidden' class applies when isFullScreen is true -->
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        ...
        <div class="process-content">
          ...
          <!-- Dynamic classes: 'active' if current, 'completed' if past -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, LLM analyzes content to auto-generate an ontology structure (Entity Types + Relation Types) suitable for opinion simulation.
                </div>
              </div>
              
              <!-- Ontology Generation Progress: Spinner shown during generation -->
              ...
              <!-- Generated Ontology Info: Show if data exists -->
              ...
              <!-- Waiting State: Shown when phase is 0 but no progress info yet -->
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Based on the generated ontology, documents are chunked and processed via Zep API to build the knowledge graph, extracting entities and relations.
                </div>
              </div>
              ...
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">Construction Progress</div>
              ...
              <!-- Construction Results Stats -->
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">Construction Results</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">Construction Completed</div>
                <div class="phase-api">Preparing for next step</div>
              </div>
          ...
          <!-- Next Step Button: Only shows when construction is fully complete -->
        ...
        <!-- Project Info Panel: Fixed at bottom of right panel -->
...
<script setup>
// Import core Vue composition API functions
import { ref, computed, onMounted, onUnmounted, watch, nextTick } from 'vue'
// Import Vue Router functions
import { useRoute, useRouter } from 'vue-router'
// Import custom API service functions
import { generateOntology, getProject, buildGraph, getTaskStatus, getGraphData } from '../api/graph'
// Import local state management for pending uploads
import { getPendingUpload, clearPendingUpload } from '../store/pendingUpload'
// Import D3.js library for graph visualization
import * as d3 from 'd3'

// Initialize router and route objects
const route = useRoute()
const router = useRouter()

// Reactive State Definitions
// Current Project ID, initialized from route params (might be 'new')
const currentProjectId = ref(route.params.projectId)

// State flags
const loading = ref(true) // General loading state for the page
const graphLoading = ref(false) // Specific loading state for graph fetching
const error = ref('') // Stores error messages
const projectData = ref(null) // Stores project metadata
const graphData = ref(null) // Stores graph nodes and edges
const buildProgress = ref(null) // Stores graph build progress (progress %, message)
const ontologyProgress = ref(null) // Stores ontology generation progress
// Phase tracker: -1 (Uploading), 0 (Generating Ontology), 1 (Building Graph), 2 (Completed)
const currentPhase = ref(-1) 
const selectedItem = ref(null) // Stores the currently clicked node or edge
const isFullScreen = ref(false) // Toggle for fullscreen graph view

// DOM element references using template refs
...
/**
 * statusClass: Returns a CSS class string based on the current phase or error state.
 * Used for coloring the status dot in the navbar.
 */
...
/**
 * statusText: Returns a user-friendly string describing the current status.
 */
const statusText = computed(() => {
  if (error.value) return 'Construction Failed'
  if (currentPhase.value >= 2) return 'Construction Completed'
...
/**
 * entityTypes: Processes raw graph nodes to extract unique entity types,
 * count their occurrences, and assign colors for the legend.
 */
const entityTypes = computed(() => {
  // Return empty array if no nodes exist
  ...
  // Iterate over nodes to count types
  graphData.value.nodes.forEach(node => {
    // Find the specific label (excluding the generic 'Entity' or 'Node' label)
    const type = node.labels?.find(l => l !== 'Entity') || 'Entity'
    if (!typeMap[type]) {
      // Initialize type entry with color from palette
...
/**
 * goHome: Navigates the user back to the root route.
 */
const goHome = () => {
  router.push('/')
}
...
/**
 * goToNextStep: Placeholder for navigating to the next workflow step.
...
/**
 * toggleFullScreen: Toggles the fullscreen state of the graph panel.
 * Waits for the CSS transition to finish before re-rendering the graph
 * to ensure correct dimensions.
 */
const toggleFullScreen = () => {
  isFullScreen.value = !isFullScreen.value
  // Wait for transition (350ms) then re-render
  setTimeout(() => {
    renderGraph()
  }, 350) 
}
...
/**
 * closeDetailPanel: Clears the selected item, effectively closing the detail panel.
 */
...
/**
 * formatDate: Converts ISO date strings into a human-readable format.
...
/**
 * selectNode: Sets the selected item to a node object.
 * Prepares data for the detail panel.
 * @param {Object} nodeData - Raw node data.
 * @param {String} color - The color assigned to this node type.
 */
const selectNode = (nodeData, color) => {
  selectedItem.value = {
    ...
    data: nodeData,
    color: color,
    // Extract specific label for the badge
    entityType: nodeData.labels?.find(l => l !== 'Entity' && l !== 'Node') || 'Entity'
  }
}

/**
 * selectEdge: Sets the selected item to an edge object.
 * @param {Object} edgeData - Raw edge data.
 */
...
/**
 * getPhaseStatusClass: Helper to get the CSS class for a phase status badge.
 * @param {Number} phase - The phase index.
 */
...
/**
 * getPhaseStatusText: Helper to get the text for a phase status badge.
 * @param {Number} phase - The phase index.
 */
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return 'Completed'
  if (currentPhase.value === phase) {
    // Special case: show percentage during build phase
...
/**
 * initProject: Entry point for component logic.
 * Determines if we are creating a new project or loading an existing one.
 */
...
/**
 * handleNewProject: Orchestrates the creation of a new project.
 * 1. Checks for pending uploads (files).
 * 2. Calls the ontology generation API.
 * 3. Starts the graph build process.
 */
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files, please return to home page'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Set phase to Ontology Generation
    ...
    if (response.success) {
      ...
      // Update local state with new project info
      currentProjectId.value = response.data.project_id
      projectData.value = response.data
      
      // Update URL to reflect the new project ID (replace history entry)
      router.replace({
        name: 'Process',
      ...
      // Proceed to Phase 1: Graph Build
...
/**
 * loadProject: Loads an existing project from the backend.
 * Restores the state based on the project's current status.
 */
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      projectData.value = response.data
      // Sync local phase with backend status
      updatePhaseByStatus(response.data.status)
      
      // Resume logic: If ontology done but graph not started
      if (response.data.status === 'ontology_generated' && !response.data.graph_id) {
        await startBuildGraph()
      }
      
      // Resume logic: If graph is currently building
      if (response.data.status === 'graph_building' && response.data.graph_build_task_id) {
        currentPhase.value = 1
        startPollingTask(response.data.graph_build_task_id)
      }
      
      // Resume logic: If graph is already complete
...
/**
 * updatePhaseByStatus: Maps backend status strings to local phase integers.
...
/**
 * startBuildGraph: Initiates the graph build process via API.
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Start periodic data fetching for the graph view
      startGraphPolling()
...
// Variable for graph data polling interval
let graphPollTimer = null

/**
 * startGraphPolling: Sets up an interval to fetch graph data periodically.
 * Useful for updating the graph in real-time during construction.
 */
...
/**
 * refreshGraph: Manual trigger to reload graph data.
 */
...
/**
 * stopGraphPolling: Cleans up the interval timer.
 */
...
/**
 * fetchGraphData: Retrieves the latest graph structure from the backend.
 * Only updates the view if data has changed (checked via node count).
 */
const fetchGraphData = async () => {
  try {
    // Fetch project to get the graph_id
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Optimization: Only render if data has changed
...
/**
 * startPollingTask: Sets up an interval to check the status of a long-running task.
 * @param {String} taskId - The ID of the background task.
 */
...
/**
 * pollTaskStatus: Checks the task status API and handles completion/failure.
 * @param {String} taskId - The ID of the background task.
 */
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Construction completed, loading graph...'
        }
...
/**
 * stopPolling: Cleans up the task status polling timer.
 */
const stopPolling = () => {
  if (pollTimer) {
...
/**
 * loadGraph: Explicitly loads graph data by ID.
...
/**
 * renderGraph: Core D3.js function to visualize the knowledge graph.
 * Handles SVG setup, force simulation, zooming, and dragging.
 */
const renderGraph = () => {
  // Guard clauses: Ensure necessary refs and data exist
  ...
  // Create a lookup map for nodes (UUID -> Node)
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Format edges for D3, filtering out invalid references
  ...
  // 'g' is a group element that will contain all graphics, allowing zoom/pan transforms
  ...
  // Visible lines for edges
  const link = linkGroup.append('line')
    .attr('stroke', '#ccc')
    .attr('stroke-width', 1.5)
    .attr('stroke-opacity', 0.6)
  
  // Transparent wider lines for easier clicking
  ...
  // Simulation Tick Function: Updates positions on every iteration
  simulation.on('tick', () => {
...
// Watcher: Re-render graph if graphData changes
watch(graphData, () => {
  if (graphData.value) {
...
/* Centered placeholders for Loading/Waiting/Error states */
.graph-loading,
.graph-waiting,
.graph-error {
  position: absolute;
...
/* Node/Edge Detail Panel (Overlay) */
.detail-panel {
  position: absolute;
  top: 16px;
  right: 16px;
...
/* Process Content Scrolling Area */
.process-content {
  flex: 1;
  overflow-y: auto;
  padding: 24px;
