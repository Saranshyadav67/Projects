...
# Adding this prefix prevents the Agent from calling tools and replies directly in text
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview prompt, adding prefix to prevent Agent from calling tools
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes that match predefined entity types (nodes where Labels is not just Entity)
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) are kept in backend/scripts/ directory, no longer copied to simulation directory
    ...
    # List of necessary files (excluding scripts, scripts are in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # If config_generated=True and files exist, consider prepared
        ...
        # - running: Currently running, means preparation finished long ago
        # - completed: Run completed, means preparation finished long ago
        # - stopped: Stopped, means preparation finished long ago
...
def prepare_simulation():
    ...
    Steps:
    1. Check if preparation work is already done
    ...
    Request (JSON):
        {
            ...
            "parallel_profile_count": 5,                  // Optional, number of parallel profile generation, default 5
    ...
    try:
        ...
        # This way frontend can get expected total Agent count immediately after calling prepare
        try:
            logger.info(f"Synchronously getting entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Quick read entities (no edge info needed, just count)
...
def get_prepare_status():
    ...
    2. Check if preparation work is already done via simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // Whether preparation is completed
                "prepare_info": {...}            // Detailed info when preparation is completed
            }
        }
    """
    ...
    try:
        ...
        # If simulation_id provided, first check if already prepared
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task:
            # Task doesn't exist, but if simulation_id provided, check if already prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverses reports directory to find reports matching simulation_id,
    returns the latest one if there are multiple (sorted by created_at)
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns simulation list containing rich info like project name, description, etc.
            ...
            "data": [
                {
                    "simulation_id": "sim_xxxx",
                    "project_id": "proj_xxxx",
                    "project_name": "Wuhan University Opinion Analysis",
    ...
    try:
        ...
        # Enrich simulation data, only read from Simulation file
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Real-time get simulation's Agent Profiles (used to view progress during generation)
    ...
    - Returns extra metadata (like file modification time, whether currently generating, etc.)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns extra metadata (like file modification time, whether currently generating, etc.)
    - Can return partial info even if config generation not complete
...
def get_simulation_config(simulation_id: str):
    ...
    Returns containing:
        ...
        - agent_configs: Each Agent's activity config (activity level, posting frequency, stance, etc.)
...
def generate_profiles():
    """
    Directly generate OASIS Agent Profiles from graph (without creating simulation)
...
def start_simulation():
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, will stop and clean run logs first
        ...
        - Suitable for scenarios needing to re-run simulation

    About enable_graph_memory_update:
        - When enabled, all Agent activities in simulation (posting, commenting, liking, etc.) will be updated to Zep graph in real-time
        - This allows graph to "remember" simulation process for subsequent analysis or AI conversation
        - Requires simulation's associated project to have valid graph_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "graph_memory_update_enabled": true,  // Whether graph memory update enabled
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation complete, check if there's running process
                ...
                # Process doesn't exist or ended, reset status to ready
...
def get_run_status_detail(simulation_id: str):
    ...
    For frontend to display real-time activity
...
def get_agent_stats(simulation_id: str):
    ...
    For frontend to display Agent activity ranking, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires simulation environment to be running (enter waiting command mode after completing simulation loop)

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // Required, simulation ID
            "agent_id": 0,                     // Required, Agent ID
            "prompt": "What's your opinion on this?",  // Required, interview question
    ...
    Returns (no platform specified, dual-platform mode):
        {
            ...
            "data": {
                "agent_id": 0,
                "prompt": "What's your opinion on this?",
                "result": {
                    "agent_id": 0,
                    "prompt": "...",
                    "platforms": {
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation has completed and entered waiting command mode."
            }), 400
...
def interview_agents_batch():
    ...
    Note: This feature requires simulation environment to be running
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                {
                    "agent_id": 0,
                    "prompt": "What's your view on A?",
                    "platform": "twitter"      // Optional, specify interview platform for this Agent
                },
                {
                    "agent_id": 1,
                    "prompt": "What's your view on B?"  // If no platform, use default
                }
            ],
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            # Validate each item's platform (if exists)
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What's your overall view on this?",  // Required, interview question (same for all Agents)
...
def close_simulation_env():
    ...
    Sends close environment command to simulation to make it gracefully exit waiting command mode.
    
    Note: This is different from /stop endpoint, /stop forcibly terminates process,
    while this endpoint makes simulation gracefully close environment and exit.
