...
def reset_project(project_id: str):
    """
    Reset project status (used for rebuilding graph)
    """
    project = ProjectManager.get_project(project_id)
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is currently building, do not submit repeatedly. To force rebuild, add force: true",
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for Zep processing to complete (query processed status of each episode)
                task_manager.update_task(
                    task_id,
