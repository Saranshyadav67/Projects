...
This module encapsulates automatic pagination logic (including single-page retries)
to transparently return a complete list to the caller.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request with exponential backoff retry on failure. Only retries network/IO transient errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Paginated fetch of graph nodes, returning at most max_items (default 2000). Each page request includes built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginated fetch of all graph edges, returning a complete list. Each page request includes built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
