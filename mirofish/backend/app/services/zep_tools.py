...
class InsightForgeResult:
    ...
    Contains retrieval results for multiple sub-questions and comprehensive analysis
    """
    query: str
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Broad Search Result (Future Panorama View)",
            f"Query: {self.query}",
            f"\n### Statistics",
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                skip = False
                # Check for both English "Question X" and Chinese "问题X" patterns
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate long content (truncate by period, not hard truncation)
                if len(clean_quote) > 150:
                    # Try to find period (Chinese or English)
                    dot_pos = clean_quote.find('\u3002') # Chinese period
                    if dot_pos < 80: dot_pos = clean_quote.find('.', 80) # English period
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            "\n### Reasoning for Selecting Interviewees",
...
class ZepToolsService:
    ...
    4. interview_agents - Deep Interview (Interviews simulated Agents, gets multi-perspective views)
    
    [Basic Tools]
    - search_graph - Graph semantic search
    - get_all_nodes - Get all graph nodes
    - get_all_edges - Get all graph edges (including time information)
    ...
    def search_graph(
        ...
        If Zep Cloud's search API is unavailable, falls back to local keyword matching.
    ...
    def _local_search(
        ...
        def match_score(text: str) -> int:
            """Calculate text and query match score"""
            if not text:
                return 0
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges of the graph (paginated fetch, including time information)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        By getting all graph edges, then filtering for edges related to the specified node
    ...
    def insight_forge(
        ...
        Most powerful hybrid retrieval function, automatically decomposes questions and retrieves multi-dimensionally:
        ...
        # Also search for original question
...
Return a JSON format list of sub-questions."""
        ...
        try:
            ...
            return [
                query,
                f"Key participants of {query}",
                f"Cause and effect of {query}",
    ...
    def panorama_search(
        ...
        3. Classifies and organizes current valid and historical information
        
        This tool is suitable for scenarios where you need to understand the full picture of an event and trace the evolution process.
        
        Args:
            graph_id: Graph ID
            query: Search query (for relevance ranking)
            include_expired: Whether to include expired content (default True)
            limit: Return result quantity limit
    ...
    def interview_agents(
        ...
        Calls real OASIS interview API to interview running Agents in the simulation:
        1. Automatically reads persona files to understand all simulation Agents
        2. Uses LLM to analyze interview requirements, intelligently selects most relevant Agents
        ...
        5. Integrates all interview results, generates interview report
        
        [Important] This feature requires the simulation environment to be running (OASIS environment not closed)
        
        [Use Cases]
        - Need to understand event views from different character perspectives
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer each question one by one, starting each answer with 'Question X:' (X is the question number)\n"
        ...
        # Step 4: Call real interview API (do not specify platform, default dual-platform simultaneous interview)
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Clean response text: remove markers, numbers, Markdown etc. interference
                ...
                # Strategy 2 (Supplementary): Correctly paired long text in Chinese or English quotes
                if not key_quotes:
                    # English quotes
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean JSON tool call wrapping in Agent response, extract actual content"""
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        """Load simulation's Agent persona files"""
        import os
        import csv
        ...
        if os.path.exists(twitter_profile_path):
            try:
                with open(twitter_profile_path, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        # CSV format converted to unified format
                        profiles.append({
...
4. Prioritize selecting roles directly related to the event
...
Please select up to {max_agents} most suitable Agents for interview and explain the reasoning."""
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = """You are a professional news editor. Please generate an interview summary based on multiple respondents' answers.

Summary requirements:
1. Extract main viewpoints from all parties
2. Point out consensus and disagreements in viewpoints
3. Highlight valuable quotes
4. Objective and neutral, not biased towards any party
5. Within 1000 words
...
- Use quotation marks "" when quoting respondents' original words
