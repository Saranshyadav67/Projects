...
Automatically generates detailed simulation parameters using LLM based on simulation requirements, document content, and graph information.
Implements full automation without manual parameter setting.

Adopts a step-by-step generation strategy to avoid failure due to generating too much content at once:
...
# China Time Schedule Configuration (Beijing Time)
...
class TimeSimulationConfig:
    ...
    # Time represented per round (simulation minutes) - Default 60 minutes (1 hour), accelerate time flow
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        """
        logger.info(f"Starting intelligent generation of simulation configuration: simulation_id={simulation_id}, entity count={len(entities)}")
    ...
    def _generate_time_config(self, context: str, num_entities: int) -> Dict[str, Any]:
        ...
        # Calculate maximum allowed value (80% of agents)
...
### Basic Principles (for reference only, should be flexibly adjusted according to specific events and participating groups):
- The user group consists of Chinese people and must conform to Beijing time schedule habits
- Early morning 0-5 has almost no activity (activity multiplier 0.05)
- Morning 6-8 gradually becomes active (activity multiplier 0.4)
- Work hours 9-18 have medium activity (activity multiplier 0.7)
- Evening 19-22 is peak period (activity multiplier 1.5)
- After 23 activity decreases (activity multiplier 0.5)
- General pattern: low activity in early morning, increasing in morning, medium during work hours, peak in evening
- **Important**: The following example values are for reference only, you need to adjust specific time periods based on event nature and participant characteristics
  - For example: Student group peak might be 21-23; Media active all day; Official institutions only during work hours
  - For example: Sudden hot events might cause discussion even late at night, off_peak_hours can be shortened appropriately
...
- peak_hours (int array): Peak hours, adjust according to event participants
...
- reasoning (string): Brief explanation of why configured this way"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format, time configuration must conform to Chinese daily habits."
...
**Important**: poster_type must be chosen from the "Available Entity Types" above, so that initial posts can be assigned to suitable Agents for publishing.
...
{{
    ...
    "reasoning": "<Brief explanation>"
        ...
        system_prompt = "You are a public opinion analysis expert. Return pure JSON format. Note poster_type must precisely match available entity types."
    ...
    def _assign_initial_post_agents(
        ...
        # Type mapping table (handling different formats LLM might output)
        type_aliases = {
        ...
        # Record used agent indices for each type to avoid reusing the same agent
...
- **Time conforms to Chinese schedule**: Almost no activity 0-5 AM, most active 19-22 PM
...
- **Individuals** (Student/Person/Alumni): High activity (0.6-0.9), mainly active in evening (18-23), fast response (1-15 minutes), low influence (0.8-1.2)
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON, configuration must conform to Chinese daily habits."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent configuration based on rules (Chinese schedule)"""
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Experts/Professors: Work + evening hours, medium frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
