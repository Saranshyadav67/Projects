...
Runs simulation in background and records each Agent's action, supporting real-time status monitoring
"""
...
# Flag indicating if cleanup function has been registered
...
class SimulationRunner:
    ...
    2. Parse run logs, record each Agent's action
    ...
    def start_simulation(
        ...
        graph_id: str = None  # Zep graph ID (required when enabling graph update)
    ) -> SimulationRunState:
        """
        Start simulation
        ...
        Args:
            ...
            graph_id: Zep graph ID (required when enabling graph update)
        ...
        try:
            ...
            # This fixes issues with third-party libraries (like OASIS) reading files without specified encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Supported in Python 3.7+, makes all open() default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set working directory to simulation directory (database etc. files will be generated here)
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform was run, check only that one
                                    # If both were run, both must complete
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines which platforms are enabled by checking if corresponding actions.jsonl files exist
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = Force, /T = Terminate process tree (including children)
            ...
            # Send SIGTERM to whole process group first
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip non-action records (like simulation_start, round_start, round_end events)
    ...
    def cleanup_all_simulations(cls):
        ...
        if not has_processes and not has_updaters:
            return  # Nothing to clean, silent return
            ...
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    try:
                        ...
                        # Process might already not exist, try direct termination
                        try:
                            process.terminate()
    ...
    def register_cleanup(cls):
        ...
        Called when Flask application starts to ensure simulation processes are cleaned up when server shuts down
        """
        ...
        # In Flask debug mode, only register cleanup in reloader child process (the one actually running the app)
        # WERKZEUG_RUN_MAIN=true indicates reloader child process
        # If not debug mode, there's no such env var, but still need to register
        ...
        # SIGHUP only exists on Unix systems (macOS/Linux), Windows doesn't have it
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Only print log if there are processes to clean
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If original handler is not callable (like SIG_DFL), use default behavior
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None: Interview both platforms in dual-platform simulation, return integrated result
    ...
    def interview_all_agents(
        ...
        Interview all Agents in simulation with the same question
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation to make it gracefully exit wait-for-command mode
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (wait for response timed out, environment may be closing)"
            }
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specified Agent ID (optional, only get that Agent's history)
