...
1. Calls Zep retrieval functionality to secondarily enrich node information
...
class OasisProfileGenerator:
    ...
    1. Calls Zep graph retrieval functionality to get richer context
    2. Generates very detailed personas (including basic info, professional experience, personality traits, social media behavior, etc.)
    3. Distinguishes between individual entities and abstract group entities
    """
    ...
    # Group/Institution entity types (need to generate representative account personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search functionality to get rich information related to the entity
        
        Zep does not have a built-in hybrid search interface, so edges and nodes need to be searched separately and results merged.
        Uses parallel requests to search simultaneously, improving efficiency.
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Determine if it is an individual entity type"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine if it is a group/institution entity type"""
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entity: Generate specific character setting
        - Group/Institution entity: Generate representative account setting
        """
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Repair truncated JSON (output truncated by max_tokens)"""
        import re
        ...
        # Simple check: if last quote isn't followed by comma or closing bracket, string might be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 2. Attempt to extract JSON part
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in social media user persona generation. Generate detailed, realistic personas for opinion simulation, maximizing restoration of existing reality. Must return valid JSON format, all string values cannot contain unescaped newlines. Use English."
        return base_prompt
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Positions and viewpoints (attitude towards topics, content that might provoke/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memories (important part of the persona, introduce this individual's connection to events, and their existing actions and reactions in the event)
...
- Use Chinese (except for gender field which must use English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account setting for an institution/group entity, maximizing restoration of existing reality.
...
2. persona: Detailed account setting description (2000 words of plain text), must include:
   ...
   - Publishing content characteristics (content type, publishing frequency, active time periods)
   - Position and attitude (official stance on core topics, handling of controversies)
   - Special notes (represented group portrait, operational habits)
   - Institution memories (important part of the institution persona, introduce this institution's connection to events, and its existing actions and reactions in the event)
3. age: Fixed to 30 (virtual age for institution account)
4. gender: Fixed to "other" (institution account uses other to indicate non-individual)
...
- Use Chinese (except for gender field which must use English "other")
- age must be integer 30, gender must be string "other"
- Institution account speech must conform to its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "age": 30,  # Institution virtual age
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID, used for Zep search to get richer context
            parallel_count: Number of parallel generations, default 5
            realtime_output_path: Real-time write file path (if provided, writes once per generation)
        ...
        # Helper function for real-time writing to file
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Real-time write to file (even if fallback persona)
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Only output to console (avoid duplication, logger does not output full content)
        print(output)
