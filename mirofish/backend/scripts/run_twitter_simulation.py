...
Features:
- Does not immediately close the environment after simulation completion; enters a command waiting mode.
...
class MaxTokensWarningFilter(logging.Filter):
    """Filters out camel-ai warnings about max_tokens (we intentionally don't set max_tokens to let the model decide)"""
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: Whether to wait for commands after simulation completion (default True)
        """
    ...
    def _create_model(self):
        ...
        """
        # Read configuration from .env with priority
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            raise ValueError("Missing API Key configuration, please set LLM_API_KEY in project root .env file")
...
async def main():
    ...
    # Initialize logging configuration (use fixed filenames, clean old logs)
...
def setup_signal_handlers():
    ...
    Give the program a chance to clean up resources properly (close databases, environments, etc.)
    """
