...
# ============================================================
# Resolve Windows encoding issues: Set UTF-8 encoding before all imports
# This is to fix issues where OASIS third-party libraries read files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # This affects all open() calls where encoding is not specified
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')
    
    # Reconfigure standard output streams to UTF-8 (fixes console Chinese character garbling)
    ...
    # Note: This needs to be set when Python starts; setting it at runtime may not work
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wraps the open() function, using UTF-8 encoding by default for text mode.
        This fixes issues where third-party libraries (like OASIS) read files without specifying encoding.
        """
        # Only set default encoding for text mode (non-binary) where encoding is not specified
...
# Script is fixed located in backend/scripts/ directory
...
class MaxTokensWarningFilter(logging.Filter):
    """Filters out camel-ai warnings about max_tokens (we intentionally don't set max_tokens, letting the model decide)"""
...
def disable_oasis_logging():
    """
    Disables verbose logging from the OASIS library.
    OASIS logs are too verbose (logging every agent's observation and action), so we use our own action_logger.
    """
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initializes simulation logging configuration
    ...
    # Clean up old log directories (if they exist)
...
class ParallelIPCHandler:
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Gets the environment and agent_graph for the specified platform
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        """
        Executes an Interview on a single platform
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        """
        Handles single Agent interview command
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/Not specified: Interview on both platforms simultaneously, return combined results
        ...
        # Platform not specified: Interview on both platforms simultaneously
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        """
        Handles batch interview command
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            print("Received environment closure command")
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Gets agent_id -> entity_name mapping from simulation_config
...
def fetch_new_actions_from_db(
    ...
    """
    Fetches new action records from the database and enriches them with complete context information
    
    Args:
        db_path: Database file path
        last_rowid: Maximum rowid value from the last read (using rowid instead of created_at because created_at formats differ across platforms)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keeping only key fields (keeping full content, not truncated)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    Enriches action with context information (post content, usernames, etc.)
    ...
    try:
        ...
        # Create comment: Add information about the post being commented on
        ...
        # Failure to enrich context does not affect main process
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, parallel simulation can use different API providers for different platforms, improving concurrency.
...
def get_active_agents_for_round(
    ...
    """Determines which Agents to activate this round based on time and configuration"""
...
async def run_twitter_simulation(
    ...
    # Get Agent real name mapping from configuration file (using entity_name instead of default Agent_X)
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed row number in database (using rowid avoids created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Log round end when no active agents (actions_count=0)
            if action_logger:
        ...
        # Get actually executed actions from database and log them
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if available, otherwise fallback to general)
...
async def main():
    ...
    # Create shutdown event at the start of main function to ensure the whole program can respond to exit signals
...
def setup_signal_handlers(loop=None):
    """
    Sets up signal handlers to ensure correct exit on SIGTERM/SIGINT
    ...
    2. Give the program a chance to clean up resources properly (close databases, environments, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (gives the loop a chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
        
        # Don't sys.exit() directly, let asyncio loop exit normally and clean up resources
        # If signal is received again, force exit
        else:
            print("Forcing exit...")
            sys.exit(1)
